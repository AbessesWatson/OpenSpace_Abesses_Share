...
RegisterNetEvent('qbx_Ab_minigame_CSC:client:local_start_devis_minigame', function(computerID)
    ...
    Wait(10)
        ...
        print ("triggered start_devis_minigame")
        SetNuiFocus(true, true)
        SetNuiFocusKeepInput(true)
        SendNUIMessage({
            action = 'openGamedevis', -- pour ouvrir l'ui
...
RegisterNUICallback('endgame', function(data, cb)
    ...
    if win == false then 
        ...
        exports.qbx_core:Notify("Tu n'as pas entré les bonnes nominations dans le devis!", 'error', 7000)
    ...
    elseif win == true then
        ...
        TriggerServerEvent('server:addProductivity', Config.upproductivity) -- Augmente la productivité
        TriggerServerEvent('server:addJobProductivity', Config.jobtoprod, Config.upproductivity) -- Augmente la productivité du job
        TriggerServerEvent('qbx_Ab_Fatigue:server:modifyPlayerFatigue', Config.lowfatigue) -- augmente la fatigue
        TriggerServerEvent("qbx_Ab_Archive_base:server:addDocToStash", "compta") -- ajoute un doc a trier pour archive
        exports.qbx_core:Notify("Tu as réussi à bien remplir le devis!", 'success', 7000)
        Wait(50) -- IMPORTANT POUR PAS DE CHEVAUCHEMENT DE SAVE DE HEALTH
...
exports.ox_target:addModel(Config.propscomputer, {
    ...
    onSelect = function(data)
        ...
        if not isGraphGameOpenEvent then
            TriggerEvent('qbx_Ab_minigame_CSC:client:local_start_devis_minigame', computerID)
            print ("trigger start_devis_minigame")
            Wait(10)
        end
    end 
    },
...
RegisterNetEvent('qbx_Ab_minigame_devis:client:blackout', function(isblackout)
    blackout = isblackout
end)
